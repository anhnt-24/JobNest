...
import { Building2, Send, Search, Bell, Volume2, Settings } from 'lucide-react';
	...
	return (
		<Card className='flex p-0 '>
			<div className='flex'>
				<div className='w-1/3 border-r'>
					...
					<div className='px-4 pb-4 flex items-center border-b'>
						<div className='relative'>
							...
							<Input type='text' placeholder='Tìm kiếm tin nhắn...' className='pl-10' />
						</div>
					</div>
					<div className='h-full'>
						{conversations.map(conv => (
							<div key={conv.id} className='p-4 border-b hover:bg-gray-50 cursor-pointer'>
								<div className='flex items-center gap-3'>
									<Avatar className='relative border w-14 h-14 '>
										<AvatarImage src={'/image.png'} className='object-cover' />
										{conv.unread > 0 && <Badge className='p-0 size-4 rounded-full absolute -bottom-1 -right-1'>{conv.unread}</Badge>}
									</Avatar>
				...
				{/* Khung chat */}
				<div className='flex-grow flex flex-col h-200'>
					{/* Header */}
					...
					<div className='p-4 border-t'>
						<div className='flex gap-2'>
							<Input type='text' placeholder='Nhập tin nhắn...' className='flex-grow' />
							<Button>
