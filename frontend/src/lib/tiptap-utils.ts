 ...
 */
export function findNodePosition(props: { editor: Editor | null; node?: TiptapNode | null; nodePos?: number | null }): { pos: number; node: TiptapNode } | null {
 ...
 */
export const handleImageUpload = async (file: File, onProgress?: (event: { progress: number }) => void, abortSignal?: AbortSignal): Promise<string> => {
	// Validate file
	if (!file) {
	...
	return 'https://cdn.tgdd.vn//GameApp/1376181//1-800x365.png';
};
